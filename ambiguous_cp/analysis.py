...
if __name__ == "__main__":
    ...
    OUTPUT_RESULT_DIR = config['chart_output_dir']
    ...
    # Load Files
    calib_plausibility_score_arr = torch.load(
        RESULTS_DIRECTORY / "calib_plausibility_score_arr")
    calib_sim_score_arr = torch.load(RESULTS_DIRECTORY / "calib_sim_score_arr")
    calib_true_class_arr = torch.load(
        RESULTS_DIRECTORY / "calib_true_class_arr")
    ...
    if ORACLE:
        ...
        calib_length = min(
            int(ratio * len(test_sim_score_arr_np)), len(calib_sim_score_arr))
        # Loop through possible per class set sizes
        for alpha in alpha_values:
            # Perform Conformal Prediction
            print("Performing Conformal Prediction")
            threshold_amb = monte_carlo_cp(
                calib_sim_score_arr[:calib_length], calib_plausibility_score_arr[:calib_length], alpha, NUM_SAMPLES)
            threshold_oracle = compute_threshold(
                alpha, test_sim_score_arr_np[:calib_length], test_true_class_arr_np[:calib_length])
            threshold_norm = compute_threshold(
                alpha, calib_sim_score_arr_np[:calib_length], calib_true_class_arr_np[:calib_length])
            # Output Metrics
            ...
            amb_metrics.append(performance_report(threshold_amb, calib_sim_score_arr_np[:calib_length], test_sim_score_arr_np[calib_length:],
                                                  calib_true_class_arr_np[:calib_length], test_true_class_arr_np[calib_length:]))
        # Generate deltas
        delta_norm = [norm_metrics[i][2]+alpha_values[i] -
                      1 for i in range(0, len(norm_metrics))]
        delta_amb = [amb_metrics[i][2]+alpha_values[i] -
                     1 for i in range(0, len(amb_metrics))]
        delta_oracle = [oracle_metrics[i][2]+alpha_values[i] -
                        1 for i in range(0, len(oracle_metrics))]
        eff_norm = [norm_metrics[i][3] for i in range(0, len(norm_metrics))]
        eff_amb = [amb_metrics[i][3] for i in range(0, len(amb_metrics))]
        eff_oracle = [oracle_metrics[i][3]
                      for i in range(0, len(oracle_metrics))]
        raw_data = {"alpha_values": alpha_values, "delta_norm": delta_norm, "delta_amb": delta_amb,
                    "delta_oracle": delta_oracle, "eff_norm": eff_norm, "eff_amb": eff_amb, "eff_oracle": eff_oracle}
    ...
    if CALIB_SIZE_CURVE:
        ...
        for pruned_size in nums_per_class:
            ...
            for i in range(0, n_calib):
                ...
                if covered[true_class] >= pruned_size:
                    continue
                if true_class > 1000:
                    continue
                ...
                pruned_plausibility_score.append(
                    calib_plausibility_score_arr[i])
            ...
            # Perform Conformal Prediction
            threshold_amb = monte_carlo_cp(
                pruned_calib_sim_score, pruned_plausibility_score, ALPHA, NUM_SAMPLES)
            ...
            test_true_class_arr_np = test_true_class_arr.detach().cpu().numpy()
            threshold_norm = compute_threshold(
                ALPHA, pruned_calib_sim_score_np, pruned_calib_true_class_np)
            # Calculate Metrics
            print("\nCalib Set Size: {Size}".format(Size=pruned_size))
            print("Normal CP:")
            norm_metric = performance_report(threshold_norm, pruned_calib_sim_score_np,
                                             test_sim_score_arr_np, pruned_calib_true_class_np, test_true_class_arr_np)
            norm_metrics.append(norm_metric)
            print("Ambiguous CP:")
            amb_metric = performance_report(threshold_amb, pruned_calib_sim_score_np,
                                            test_sim_score_arr_np, pruned_calib_true_class_np, test_true_class_arr_np)
            amb_metrics.append(amb_metric)

        plt.plot(nums_per_class, [
                 norm_metric[2] for norm_metric in norm_metrics], color='blue', label='normal')
        plt.plot(nums_per_class, [
                 amb_metric[2] for amb_metric in amb_metrics], color='red', label='ambiguous')
        plt.axhline(y=1-ALPHA, color='green', linestyle='-')
        plt.title(
            "Calib Size Per Class v. Test Coverage Curve: {alpha}".format(alpha=ALPHA))
        plt.xlabel("# Calib Instances Per Class")
        plt.ylabel("Coverage (proportion of true labels covered)")
        plt.legend()
        plt.savefig(OUTPUT_RESULT_DIR /
                    "Coverage_Size-{alpha}.png".format(alpha=int(ALPHA*100)))
        plt.show()

        plt.plot(nums_per_class, [
                 norm_metric[3] for norm_metric in norm_metrics], color='blue', label='normal')
        plt.plot(nums_per_class, [
                 amb_metric[3] for amb_metric in amb_metrics], color='red', label='ambiguous')
        plt.title(
            "Calib Size Per Class v. Test Efficiency Curve: {alpha}".format(alpha=ALPHA))
        plt.xlabel("# Calib Instances Per Class")
        plt.ylabel("Efficiency (mean num of extraneous classes per sample)")
        plt.legend()
        plt.savefig(OUTPUT_RESULT_DIR /
                    "Efficiency_Size-{alpha}.png".format(alpha=int(ALPHA*100)))
        plt.show()
    ...
    if ALPHA_CURVE:
        ...
        for alpha in alpha_values:
            ...
            threshold_norm = compute_threshold(
                alpha, calib_sim_score_arr_np, calib_true_class_arr_np)
            # Output Metrics
            print("\nAlpha Value: {alpha}".format(alpha=alpha))
            print("Normal CP:")
            norm_metrics.append(performance_report(threshold_norm, calib_sim_score_arr_np,
                                test_sim_score_arr_np, calib_true_class_arr_np, test_true_class_arr_np))
            print("Ambiguous CP:")
            amb_metrics.append(performance_report(threshold_amb, calib_sim_score_arr_np,
                               test_sim_score_arr_np, calib_true_class_arr_np, test_true_class_arr_np))
        # Generate deltas
        delta_norm = [norm_metrics[i][2]+alpha_values[i] -
                      1 for i in range(0, len(norm_metrics))]
    ...
    if UNCERTAIN_HIST:
        ...
        # Invert Softmax
        calib_sim_score_arr_np = (
            np.log(calib_sim_score_arr_np) + LOGIT_SCALE)/LOGIT_SCALE
        test_sim_score_arr_np = (
            np.log(test_sim_score_arr_np) + LOGIT_SCALE)/LOGIT_SCALE
        for i in range(0, n_calib):
            # Append scores
            true_class_calib_scores.append(
                np.dot(calib_sim_score_arr_np[i], calib_true_class_arr_np[i]))
            true_class_test_scores.append(
                np.dot(test_sim_score_arr_np[i], test_true_class_arr_np[i]))
            ...
            plausibility_expected_scores.append(
                np.dot(calib_sim_score_arr_np[i], plausibility_point))
            plausibility_max_scores.append(
                (calib_sim_score_arr_np[i][max_at] * plausibility_point[max_at]))

        raw_data = {"true_class_test_scores": true_class_test_scores, "plausibility_expected_scores": plausibility_expected_scores,
                    "true_class_calib_scores": true_class_calib_scores, "plausibility_max_scores": plausibility_max_scores}
        with open(OUTPUT_RESULT_DIR / "Histogram_Comparison.pkl", 'wb') as f:
            pickle.dump(raw_data, f)

        # Generate Histogram
        bins = [0.01*i for i in range(0, 100)]
        plt.hist(plausibility_expected_scores, bins=bins, density=True,
                 alpha=0.5, label=u'Expected Scores $E(x, λ)$', color='blue')
        plt.hist(true_class_test_scores, bins=bins, density=True,
                 alpha=0.5, label=u'Test True Scores $E(x, y)$', color='green')
        plt.xlabel('Conformity Score')
        plt.ylabel('Frequency')
        plt.title(u'Expected Scores $E(x, λ)$ Histogram')
        plt.legend()
        plt.savefig(OUTPUT_RESULT_DIR /
                    "Conformity_Histogram_Expected-Plausibility.png")
        plt.show()

        plt.hist(true_class_calib_scores, bins=bins, density=True,
                 alpha=0.5, label=u'Calib True Scores $E(x, y)$', color='red')
        plt.hist(true_class_test_scores, bins=bins, density=True,
                 alpha=0.5, label=u'Test True Scores $E(x, y)$', color='green')
        plt.xlabel('Conformity Score')
        plt.ylabel('Frequency')
        ...
        plt.hist(plausibility_max_scores, bins=bins, density=True, alpha=0.5,
                 label=u'Top-1 Expected Scores $E(x, argmax_{k}λ_{k})$', color='purple')
        plt.hist(true_class_test_scores, bins=bins, density=True,
                 alpha=0.5, label=u'Test True Scores $E(x, y)$', color='green')
        plt.xlabel('Conformity Score')
        plt.ylabel('Frequency')
        ...
        plt.legend()
        plt.savefig(OUTPUT_RESULT_DIR /
                    "Conformity_Histogram_Argmax-Plausibility.png")
        plt.show()
    ...
    if PLAUSIBILITY_HISTOGRAM:
        ...
        plt.hist(true_plausibilities, bins=bins, density=True,
                 alpha=0.5, label=u'$λ_{y}$', color='green')
        plt.xlabel(u'Plausibility Score (λ)')
        plt.ylabel('Frequency')
        plt.title(u'Intended Class Plausibility Distribution')
        plt.legend()
        plt.savefig(OUTPUT_RESULT_DIR /
                    "True_Class_Histogram-Plausibility.png")
        plt.show()

        plt.hist(junk_plausibilities, bins=bins, density=True,
                 alpha=0.5, label=u'$λ_{j}$', color='red')
        plt.xlabel(u'Plausibility Score (λ)')
        plt.ylabel('Frequency')
        plt.title(u'Junk Plausibility Distribution')
        plt.legend()
        plt.savefig(OUTPUT_RESULT_DIR /
                    "Junk_Class_Histogram-Plausibility.png")
        plt.show()

        plt.hist(other_plausibilities, bins=bins, density=True, alpha=0.5,
        ...
        plt.legend()
        plt.savefig(OUTPUT_RESULT_DIR /
                    "Other_Class_Histogram-Plausibility.png")
        plt.show()
